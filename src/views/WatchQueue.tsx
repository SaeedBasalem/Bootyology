...
import { Plus, Play, CheckCircle2, Link, Film, Eye, Search, Upload, X } from 'lucide-react'
  ...
  return (
    <div className={classNames(
      ...
      <div className="relative h-32 bg-surface2 flex items-center justify-center">
        ...
        {clip.url && (
          <a href={clip.url} target="_blank" rel="noreferrer" className="absolute inset-0 flex items-center justify-center bg-black/60 opacity-0 group-hover:opacity-100 transition">
...
function AddToQueueModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function handleSave() {
    if (!title.trim()) {
      toast({ title: 'Add a title', icon: '⚠️' })
      return
    }
    if (tab === 'link' && !url.trim()) {
      toast({ title: 'Add a URL', icon: '⚠️' })
      return
    }
    saveClip({
      modelId: modelId || '__unlinked__',
      title: title.trim(),
      source: tab === 'file' ? 'file' : 'link',
    ...
    setTitle(''); setUrl(''); setModelId('__unlinked__')
    onClose()
  }
      ...
      <div className="w-full max-w-md rounded-2xl border border-line bg-surface p-6 shadow-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-4 flex gap-2">
          {(['link', 'file'] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} className={classNames('btn flex-1 py-2 text-xs', tab === t ? 'btn-cm' : 'btn-ghost')}>
              {t === 'link' ? <Link size={13} /> : <Upload size={13} />}
              {t === 'link' ? 'URL / Link' : 'Description only'}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input className="input" placeholder="Describe the clip…" value={title} onChange={(e) => setTitle(e.target.value)} autoFocus />
          </div>
          {tab === 'link' && (
            <div>
          ...
          <div>
            <label className="label">Link to model (optional)</label>
        ...
        <div className="mt-5 flex gap-2">
          ...
          <button className="btn-cm flex-1" onClick={handleSave}><Plus size={15} /> Add to queue</button>
        </div>
      </div>
    </div>
  )
...
export function WatchQueue() {
  ...
  // All clips tagged as queue items (have watchStatus) or modelId === '__unlinked__'
  let queueClips = data.clips.filter((c) => c.watchStatus !== undefined || c.modelId === '__unlinked__')
    .slice().sort((a, b) => (a.queuePriority ?? 0) - (b.queuePriority ?? 0) || b.createdAt.localeCompare(a.createdAt))
  ...
  const counts = {
    all: data.clips.filter((c) => c.watchStatus !== undefined || c.modelId === '__unlinked__').length,
    unwatched: data.clips.filter((c) => (c.watchStatus ?? 'unwatched') === 'unwatched' && (c.watchStatus !== undefined || c.modelId === '__unlinked__')).length,
    watched: data.clips.filter((c) => c.watchStatus === 'watched').length,
    scored: data.clips.filter((c) => c.watchStatus === 'scored').length,
    watching: data.clips.filter((c) => c.watchStatus === 'watching').length,
  }
    ...
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {(['all', 'unwatched', 'watching', 'watched', 'scored'] as const).map((s) => (
          ...
          >
            {s === 'all' ? 'All' : STATUS_LABELS[s].label} <span className="ml-0.5 text-muted">{counts[s]}</span>
          </button>
        ))}
      ...
      {queueClips.length === 0 ? (
        <EmptyState
          ...
          message={counts.all === 0
            ? 'Add clips you want to watch and judge. You can link them to a model before or after watching.'
            : 'Try a different filter or search.'}
          action={counts.all === 0 ? <button className="btn-cm" onClick={() => setAddOpen(true)}><Plus size={15} /> Add first clip</button> : undefined}
        />
      ) : (
