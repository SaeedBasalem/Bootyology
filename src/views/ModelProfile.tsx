...
import { ArrowLeft, Pencil, ClipboardPlus, Heart, Trash2, Trophy, Calendar, Film, Plus } from 'lucide-react'
...
export function ModelProfile() {
  ...
  if (!model) {
    return (
      <EmptyState
        ...
        action={
          <button className="btn-gold" onClick={() => go('models')}>
            Back to roster
          </button>
        }
      />
  ...
  // strengths / weaknesses from latest card
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card relative overflow-hidden p-6">
        <div
          className="pointer-events-none absolute -right-10 -top-10 h-48 w-48 rounded-full opacity-25 blur-2xl"
          style={{ background: `radial-gradient(circle, ${model.accent}, transparent 70%)` }}
        />
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center">
          <Avatar name={model.name} emoji={model.emoji} accent={model.accent} size={76} />
          <div className="flex-1">
            ...
            {model.aliases && <p className="text-sm text-muted">aka {model.aliases}</p>}
        ...
        {model.notes && <p className="relative mt-4 rounded-xl bg-surface2 p-3 text-sm text-muted">{model.notes}</p>}
      </div>
      ...
      <div className="card p-5">
        ...
        {modelClips.length === 0 ? (
          <p className="rounded-xl bg-surface2 p-4 text-center text-sm text-muted">
            No clips linked yet. Attach a clip you have for {model.name} — then score it directly from the player.
          </p>
        ) : (
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            <span className="font-display text-2xl font-bold" style={{ color: tier.color }}>
                              {card.total}
                            </span>
                          ...
                          {confirmDelete === card.id ? (
                            <button onClick={() => handleDelete(card.id)} className="btn-danger h-8 px-2 text-xs">
                              Sure?
                            </button>
                          ) : (
                        ...
                        (() => {
                          ...
                          return (
                            <button
                              ...
                              className="mt-3 inline-flex items-center gap-2 rounded-lg bg-surface px-3 py-1.5 text-xs font-medium text-gold transition hover:bg-surface2"
                            >
    ...
    <div>
      <div className="mb-1 flex items-center justify-between text-sm">
        ...
        <span className="text-muted">
          {value}/{c.isDeduction ? c.min : c.max}
        </span>
      </div>
